...
// GPT-4o-mini-audio for TTS. AI content commands. Doubao→Gemini→Haiku routing.
...
async function executeCommandFromBackground(command, settings) {
  ...
  try {
    ...
    const tierLabel = { fast:'Doubao', standard:'Gemini', haiku:'Haiku', fallback:'AI' }[aiResult.tier] || 'AI';
    if (cr.success) {
...
const DOUBAO_PROMPT = `Browser voice nav AI. Map command to best UI element.
...
async function route(command,uiString,apiKey,endpoint){
  ...
  let d=null,g=null;
  try{const raw=await callModel(EP.DOUBAO,DOUBAO_PROMPT,userMsg,apiKey,endpoint,80,8000);d=normalize(raw);if(d.action==='click'&&d.confidence>=0.75)return{...d,tier:'fast'};if(d.action==='keypress')return{...d,tier:'fast'};}
  catch(e){if(isConfigErr(e))throw e;}
...
async function testConnection(apiKey,endpoint){
  try{const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${apiKey}`},body:JSON.stringify({model:EP.DOUBAO,max_tokens:5,temperature:0,messages:[{role:'user',content:'hi'}]})});
  if(r.status===401)return{ok:false,msg:ERRS.KEY};if(r.status===403)return{ok:false,msg:ERRS.FORBIDDEN};if(r.status===404)return{ok:false,msg:ERRS.NOT_FOUND};if(r.status===429)return{ok:true,msg:'✓ Connected (rate limited but key works)'};if(!r.ok){const b=await r.text().catch(()=>'');return{ok:false,msg:`API ${r.status}: ${b.slice(0,80)}`};}return{ok:true,msg:'✓ Connected! Doubao → Gemini → Haiku ready.'};}
